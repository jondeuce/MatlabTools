...
persistent gif_filename firstframe DelayTime DitherOption LoopCount Scale frame %jd
...
if nargin>0 
   ...
   if any(tmp) 
      ...
      assert(isscalar(Scale)==1,'Error: Scale must be a scalar value.') 
   end
...
% Get frame: %jd
f = getframe(frame.Parent);
f.cdata = imresize(f.cdata, Scale);
